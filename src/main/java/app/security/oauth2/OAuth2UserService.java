...
import app.security.oauth2.user.OAuth2UserInfo;
import app.security.oauth2.user.OAuth2UserInfoFactory;
...
public class OAuth2UserService extends DefaultOAuth2UserService {
  ...
  private OAuth2User processOAuth2User(OAuth2UserRequest oAuth2UserRequest, OAuth2User oAuth2User) throws Exception {
    ...
    if (userOptional.isPresent()) {
      ...
      if (!user.getPlatform().equals(Platform.valueOf(oAuth2UserRequest.getClientRegistration().getRegistrationId()))) {
        throw new Exception();
      }
    ...
    return new Auth(user, oAuth2User.getAttributes());
  }
  ...
  private User createUser(OAuth2UserRequest oAuth2UserRequest, OAuth2UserInfo oAuth2UserInfo) {
    ...
    user.setPlatformToken(oAuth2UserInfo.getId());
